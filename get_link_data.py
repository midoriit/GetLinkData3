...
class GetLinkData:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&GetLinkData3')
    ...
    def run(self):
        ...
        if result:
            ...
            url2 = "http://linkdata.org/api/1/" + id + "/" + \
              urllib.parse.quote(file.encode("utf-8")) + "_rdf.json"
            ...
            if not lat.startswith("http"):
              lat = "http://linkdata.org/property/" + id + "#" + urllib.parse.quote(lat.encode("utf-8"))
            if not lon.startswith("http"):
              lon = "http://linkdata.org/property/" + id + "#" + urllib.parse.quote(lon.encode("utf-8"))
    ...
    def setID(self):
      ...
      try:
        rsp1 = urllib.request.urlopen(req1)
      except urllib.error.HTTPError as e:
      ...
      for rs in self.dat1["resources"]:
        if "url" in rs:
          ...
          file_list.append(urllib.parse.unquote(csv[s:len(csv)-4]).encode("raw_unicode_escape").decode("utf-8"))
        else:
